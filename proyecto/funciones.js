...
function visualizarCarrito() {
    ...
    carrito.forEach(producto => {
        ...
        const precioElement = document.createElement('p');
        precioElement.innerText = `Precio total: $${(producto.precio * producto.cantidad)}`; // cálculo el total
...
function eliminarDelCarrito(idProducto) {
    ...
    if (indexProducto >= 0) {
        carrito.splice(indexProducto, 1); // indexproducto es donde debo eliminar , el 1 es el elemento a eliminar
        guardarCarrito(); // Actualiza 
        visualizarCarrito(); //  visualizar el carrito actualizado
    }

   alert("tu carrito se elimino o actualizo", carrito);
}